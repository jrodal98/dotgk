...
pub fn sync_command(cache_path: Option<PathBuf>, force: bool) -> Result<()> {
    ...
    if settings.cache_formats.lua {
        if let Err(e) = generate_lua_cache(&cache, None) {
            error!("Failed to generate Lua cache: {}", e);
        } else {
            generated_formats.push("Lua");
        }
    }

    if settings.cache_formats.shell {
        if let Err(e) = generate_shell_cache(&cache, None) {
            error!("Failed to generate shell cache: {}", e);
        } else {
            generated_formats.push("shell");
        }
    }
...
fn generate_lua_cache(cache: &Cache, cache_path: Option<PathBuf>) -> Result<()> {
    let mut config_dir = get_config_dir()?;
    config_dir.push("cache");
    config_dir.push("cache.lua");

    let lua_cache_path = cache_path.unwrap_or(config_dir);

    // Create cache directory if it doesn't exist
    if let Some(parent) = lua_cache_path.parent() {
        fs::create_dir_all(parent)?;
    }

    let mut lua_content = String::new();
    lua_content.push_str("-- Auto-generated by dotgk sync\n");
    lua_content.push_str("-- Do not edit manually\n\n");
    lua_content.push_str("local dotgk = {\n");
    ...
    lua_content.push_str("return dotgk\n");

    fs::write(&lua_cache_path, lua_content)
        .with_context(|| format!("Failed to write Lua cache to {:?}", lua_cache_path))?;

    info!("Generated Lua cache at {:?}", lua_cache_path);
    Ok(())
}

fn generate_shell_cache(cache: &Cache, cache_path: Option<PathBuf>) -> Result<()> {
    let mut config_dir = get_config_dir()?;
    config_dir.push("cache");
    config_dir.push("cache.sh");

    let shell_cache_path = cache_path.unwrap_or(config_dir);

    // Create cache directory if it doesn't exist
    if let Some(parent) = shell_cache_path.parent() {
        fs::create_dir_all(parent)?;
    }

    let mut shell_content = String::new();
    shell_content.push_str("#!/bin/bash\n");
    shell_content.push_str("# Auto-generated by dotgk sync\n");
    shell_content.push_str("# Do not edit manually\n");
    shell_content.push_str("# Source this file to get gatekeeper variables\n\n");
    ...
    for (name, entry) in entries {
        ...
        shell_content.push_str(&format!("export DOTGK_{}=\"{}\"\n", var_name, value));
    }

    // Add a helper function to check gatekeeper values
    shell_content.push_str("\n# Helper function to check gatekeeper values\n");
    shell_content.push_str("dotgk_check() {\n");
    shell_content.push_str("  local name=\"$1\"\n");
    shell_content.push_str("  local var_name=\"DOTGK_$(echo \"$name\" | tr '[:lower:]' '[:upper:]' | tr '-.' '__')\"\n");
    shell_content.push_str("  local value=\"${!var_name}\"\n");
    shell_content.push_str("  [[ \"$value\" == \"true\" ]]\n");
    shell_content.push_str("}\n");

    fs::write(&shell_cache_path, shell_content)
        .with_context(|| format!("Failed to write shell cache to {:?}", shell_cache_path))?;

    info!("Generated shell cache at {:?}", shell_cache_path);
    Ok(())
}
